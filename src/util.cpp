...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Latinio";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "latinio.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "latiniod.pid"));
