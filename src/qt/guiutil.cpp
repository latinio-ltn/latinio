...
{
    // Convert latinio:// to latinio:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("latinio://"))
    {
        uri.replace(0, 12, "latinio:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Latinio.lnk";
}
...
{
    return GetAutostartDir() / "latinio.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Latinio-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  latinio-qt [" + tr("command-line options") + "]                     " + "\n";
...
void SetBlackThemeQSS(QApplication& app)
{
    app.setStyleSheet("QWidget        { background: rgb(255,255,255); }"
                      ...
                      "QDoubleSpinBox { background: rgb(255,255,255); color: rgb(28,28,28); border-color: rgb(251,192,45); }"
                      "QLineEdit      { background: rgb(255,255,255); color: rgb(28,28,28); border-color: rgb(251,192,45); }"
